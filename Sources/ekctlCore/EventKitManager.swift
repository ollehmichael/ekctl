...
public class EventKitManager {
    ...
    /// Lists events in a calendar within a date range, with optional keyword search
    public func listEvents(calendarID: String, from startDate: Date, to endDate: Date, search: String? = nil) -> JSONOutput {
        guard let calendar = eventStore.calendar(withIdentifier: calendarID) else {
            return JSONOutput.error("Calendar not found with ID: \(calendarID)")
        }
        ...
        if let keyword = search, !keyword.isEmpty {
    ...
    public func updateEvent(
        ...
        if let location = location {
            ...
            event.structuredLocation = resolveLocation(location)
        }
    ...
    public func addEvent(
        ...
        if let locationString = location {
            ...
            event.structuredLocation = resolveLocation(locationString)
        }
    ...
    /// Creates a date formatter that outputs ISO 8601 format with timezone offset
    ...
    private func eventToDict(_ event: EKEvent) -> [String: Any] {
        ...
        dict["location"] = event.location.flatMap { $0.isEmpty ? nil : $0 } as Any? ?? NSNull()
        dict["notes"] = event.notes.flatMap { $0.isEmpty ? nil : $0 } as Any? ?? NSNull()
        ...
        // Full recurrence rules serialization
        if let rules = event.recurrenceRules, !rules.isEmpty {
            dict["recurrenceRules"] = rules.map { ruleToDict($0) }
        } else {
            dict["recurrenceRules"] = [] as [[String: Any]]
        }
    ...
    /// Serializes an EKRecurrenceRule to a dictionary
    private func ruleToDict(_ rule: EKRecurrenceRule) -> [String: Any] {
        var dict: [String: Any] = [
        ...
        if let days = rule.daysOfTheWeek, !days.isEmpty {
            dict["daysOfTheWeek"] = days.map { d -> [String: Any] in
                var entry: [String: Any] = ["day": weekdayString(d.dayOfTheWeek)]
                if d.weekNumber != 0 { entry["weekNumber"] = d.weekNumber }
                return entry
            }
        }
    ...
    private func reminderToDict(_ reminder: EKReminder) -> [String: Any] {
        ...
        dict["notes"] = reminder.notes.flatMap { $0.isEmpty ? nil : $0 } as Any? ?? NSNull()
