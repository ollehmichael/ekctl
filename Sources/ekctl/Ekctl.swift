...
struct ListEvents: ParsableCommand {
    ...
    @Option(name: .long, help: "Filter events by keyword (case-insensitive match on title, notes, location).")
    var search: String?
    ...
    func run() throws {
        ...
        let result = manager.listEvents(calendarID: calendarID, from: startDate, to: endDate, search: search)
        print(result.toJSON())
    }
}
...
struct AddEvent: ParsableCommand {
    ...
    @Option(name: .long, help: "Alarms in minutes before start (comma-separated). Prefix '+' for after.")
    var alarms: String?
    ...
    func run() throws {
        ...
        if let priority = priority {
            guard let p = Int(priority) else {
                print(JSONOutput.error("Invalid --priority value. Must be an integer (e.g., 0,1,5,9).").toJSON())
                throw ExitCode.failure
            }
            priorityInt = p
        }
...
struct UpdateReminder: ParsableCommand {
    ...
    func run() throws {
        ...
        if let priority = priority {
            guard let p = Int(priority) else {
                print(JSONOutput.error("Invalid --priority value. Must be an integer (0, 1, 5, or 9).").toJSON())
                throw ExitCode.failure
            }
            priorityInt = p
        }
